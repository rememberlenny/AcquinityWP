 ...
 * @version 1.3 to 2.0 to support OAuth2 by Vertazzar / Edis Sehalic 
 * @link https://www.elance.com/s/vertazzar/
 * 
 */
...
class gapi
{
  ...
  // listing accounts with v3
  const account_data_url = 'https://www.googleapis.com/analytics/v3/management/accounts/~all/webproperties/~all/profiles'; //OLD https://www.google.com/analytics/feeds/accounts/default
  // report data used 2.4 version for XML backwards compatibility
  ...
  {
    $response = $this->httpRequest(gapi::account_data_url, array('start-index'=>$start_index, 'access_token' => $this->auth_token, 'max-results'=>$max_results), null, null);
  ...
  public function requestReportData($report_id, $dimensions, $metrics, $sort_metric=null, $filter=null, $start_date=null, $end_date=null, $start_index=1, $max_results=30)
  {
   ...
   // $parameters['alt'] = 'xml';
    ...
    $response = $this->httpRequest(gapi::report_data_url, $parameters, null, null);
  ...
  protected function accountObjectMapper($xml_string)
  {
	...
	$this->results = null;
	$d = json_decode($xml_string, true);
    foreach($d['items'] as $entry)
    {
      $results[] = new gapiAccountEntry($entry);
    }
