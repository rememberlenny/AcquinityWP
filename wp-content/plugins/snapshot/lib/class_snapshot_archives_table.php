		...
		function column_file($data_item) {
			...
			if ((empty($data_item['destination'])) || ($data_item['destination'] == "local")) {
				if (isset($data_item['filename'])) {
					$current_backupFolder = $this->_parent->snapshot_get_item_destination_path($this->item, $data_item);
					if (empty($current_backupFolder)) {
						$current_backupFolder = $this->_parent->snapshot_get_setting('backupBaseFolderFull');
					}
...
>>>>>>> 2c051ad76c4e79b45eb2ffa1c0d1b03b8e2c09b9
		...
		function column_logs($data_item) {
			...
			$backupLogFileFull = trailingslashit($this->_parent->snapshot_get_setting('backupLogFolderFull'))
